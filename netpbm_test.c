...
{
	int m, n, intValue;
	double dblValue, minVal = 255, maxVal = -255;
...
Image blurring(Image inputImage,Image blurrImage){
    int x, y;
    double intensity;
    for (x = 1; x < inputImage.width-1; x++)
		...
		{
		    intensity=0;
            intensity+=inputImage.map[y][x].i;
            intensity+=inputImage.map[y+1][x].i;
            intensity+=inputImage.map[y-1][x].i;
            intensity+=inputImage.map[y][x+1].i;
            intensity+=inputImage.map[y][x-1].i;
            intensity+=inputImage.map[y-1][x-1].i;
            intensity+=inputImage.map[y+1][x-1].i;
            intensity+=inputImage.map[y+1][x+1].i;
            intensity+=inputImage.map[y-1][x+1].i;
            intensity=intensity/9;
            blurrImage.map[y][x].i=intensity;
        }
    for (x = 0; x < blurrImage.width; x++)
		for (y = 0; y < blurrImage.height; y++)
		{
		    if(x==0 || y==0 || x==blurrImage.width-1 ||y==blurrImage.height-1){
                blurrImage.map[y][x].i=255;
		    }
		    if(blurrImage.map[y][x].i<128){
                blurrImage.map[y][x].i=0;
		    }
		    else{
                blurrImage.map[y][x].i=255;
		    }
...
Image expansion(Image inputImage,Image expandedImage){
    int x, y;
    for (x = 0; x < inputImage.width; x++)
		for (y = 0; y < inputImage.height; y++)
		{
            if(inputImage.map[y][x].i==0){
                    ...
                    expandedImage.map[y][x].i=inputImage.map[y][x].i;
                    if(y+1<inputImage.height && inputImage.map[y+1][x].i==255){
                        expandedImage.map[y+1][x].i=inputImage.map[y][x].i;
                    }
                    if(y-1>=0 && inputImage.map[y-1][x].i==255){
                        expandedImage.map[y-1][x].i=inputImage.map[y][x].i;
                    }
                    if(x+1<inputImage.width && inputImage.map[y][x+1].i==255){
                        expandedImage.map[y][x+1].i=inputImage.map[y][x].i;
                    }
                    if(x-1>=0 && inputImage.map[y][x+1].i==255){
                    expandedImage.map[y][x-1].i=inputImage.map[y][x].i;
                    }
            }
		}
...
Image shrink(Image inputImage,Image shrinkImage){

    int x, y;
    for (x = 0; x < inputImage.width; x++)
		for (y = 0; y < inputImage.height; y++)
		{
            if(inputImage.map[y][x].i==255){
                    shrinkImage.map[y][x].i=255;
                    if(y+1<inputImage.height && inputImage.map[y+1][x].i==0){
                        shrinkImage.map[y+1][x].i=255;
                    }
                    if(y-1>=0 && inputImage.map[y-1][x].i==0){
                        shrinkImage.map[y-1][x].i=255;
                    }
                    if(x+1<inputImage.width && inputImage.map[y][x+1].i==0){
                        shrinkImage.map[y][x+1].i=255;
                    }
                    if(x-1>=0 && inputImage.map[y][x-1].i==0){
                    shrinkImage.map[y][x-1].i=255;
                    }

            }
            else{
                shrinkImage.map[y][x].i=inputImage.map[y][x].i;
            }
...
void main()
{
	Image blurrImage, expandedImage, inputImage,shrinkImage;
    inputImage = readImage("text_image.pbm");

    expandedImage = createImage(inputImage.height, inputImage.width);
	shrinkImage=createImage(inputImage.height,inputImage.width);

    shrinkImage=shrink(inputImage,shrinkImage);
    expandedImage=expansion(shrinkImage,expandedImage);
    shrinkImage=shrink(expandedImage,shrinkImage);
    expandedImage=expansion(shrinkImage,expandedImage);

	blurrImage = createImage(inputImage.height, inputImage.width);

    blurrImage=blurring(inputImage,blurrImage);


    writeImage(expandedImage,"text_cleaned_1.pbm");
    writeImage(blurrImage,"text_cleaned_2.pbm");
    deleteImage(expandedImage);
    deleteImage(shrinkImage);
	deleteImage(blurrImage);
}
